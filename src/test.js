            ...
            case 302: {
              switch (res.headers.location) {
                ...
                case insta + 'accounts/login/?next=/accounts/edit/%3F__a%3D1':
                  return reject(401);
                default: {
                  if (res.headers.location.startsWith(insta + 'challenge/?next=')) { return reject(409); }
...
module.exports = class Insta {
  ...
  authBySessionId(sessionId) {
    return new Promise((resolve, reject) => self.get('accounts/edit', sessionId)
      .then(body => {
        if (this.sessionId) { process.emitWarning('Session ID changed'); }
  ...
  async _getProfileId(username) {
    if (!profileIds[username]) { profileIds[username] = (await this.getProfile(username)).id; }
  ...
  async _getQueryHashs() {
    ...
    const secondaryScriptBody = await self.get(ConsumerLibCommons, undefined, false);
  ...
  async getPostImageUrls(shortcode) {
    const [post] = await self.get(`p/${shortcode}`, this.sessionId);
    return post?.image_versions2?.candidates;
  }
